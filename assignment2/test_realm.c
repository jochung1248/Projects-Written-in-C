...
void test_add_location(void) {
    ...
    num_locations = add_location(test_realm, "1.Location");
    num_locations = add_location(test_realm, "2.Location");
    num_locations = add_location(test_realm, "3.Location");
    num_locations = add_location(test_realm, "4.Location");
...
void test_new_enemy(void) {
    ...
    new_enemy(test_realm, "Lair", "Enemy", 5);
    
    char print_text[10000];
    CAPTURE(print_realm(test_realm), print_text, 10000);

    if (!string_contains(print_text, "Enemy [hp: 5/5]")) {
        printf(DOES_NOT_FOLLOW_SPEC);
        // since we don't want to print FOLLOWS_SPEC at the end,
        // we just return to the main function here.
        return;
...
void test_new_tower(void) {
    ...
    new_tower(test_realm, "Castle", "Tower1", 5, 5);
    new_tower(test_realm, "Tower1", "Tower2", 5, 5);
    
    char print_text[10000];
    CAPTURE(print_realm(test_realm), print_text, 10000);

    if ((!string_contains(print_text, "Tower1 [pow:   5 | uses:   5] {n}")) &&
         (!string_contains(print_text, "Tower2 [pow:   5 | uses:   5] {n}"))) {
        printf(DOES_NOT_FOLLOW_SPEC);
        // since we don't want to print FOLLOWS_SPEC at the end,
        // we just return to the main function here.
        return;
